...
class TestFaceDetectorFactory(unittest.TestCase):
    ...
    def test_learnFaceDetector(self, saveDetector=False, test=False, saveIntermediates=False, debug=True):
            ...
            for t in range(T):
                ...
                for k in range(K):
                    for i in range(N):
                        ...
                        self.assertEqual(residuals[i].tolist(),
                                renormalize(shapeDeltas[i] - meanDelta, imageAdapters[pi[i]]).tolist())
